...
Original file is located at
    https://colab.research.google.com/drive/1J6R20SSRdx9y8GMyiayYlaMnrQVBOvaa

# Introduction by Blas Facundo Ferreira, Data Scientist:

Greetings,

I am Blas Facundo Ferreira, a dedicated data scientist with a deep-rooted passion for delving into the realms of machine learning and its multifaceted applications in Natural Language Processing (NLP).

**LinkedIn**: https://www.linkedin.com/in/blasferreira/

**Portfolio**: https://portfolioblasferreira.streamlit.app/

**GitHub**: https://github.com/BlasFerreira

Today, I present to you a comprehensive Machine Learning Task tailored for the Machine Learning Engineer role.

# Summary :

**Data Extraction:** I used the BeautifulSoup and requests libraries to extract content from an article on "The Guardian".

**Entity Recognition:** I applied a BERT model from Hugging Face for Named Entity Recognition (NER), identifying key entities such as names, places, and organizations.

**Text Summarization:** I segmented the content and used a BART model to generate concise summaries for each segment.

**Sentiment Analysis:** I employed a model from CardiffNLP to determine the tone of the article, assessing whether the content was positive, negative, or neutral.

**Emotional Evaluation:** To analyze the emotions conveyed in the article, I utilized the "SamLowe/roberta-base-go_emotions" model from Hugging Face. This model is designed to identify a range of emotions in the text, such as joy, sadness, anger, among others.

**Question Generator:** I've developed a question generator that, using NLP techniques and machine learning models, automatically crafts questions based on a given context.

Through this notebook, I conducted a detailed analysis of the article's content using various NLP tools and techniques.
...
I used the **Python** libraries **BeautifulSoup** and **requests** to extract content from an article on **"The Guardian"**. I started a request session and set specific headers that mimic a real web browser, which is sometimes necessary to access restricted content. With these headers set up, I made an HTTP request to the article and processed the response with BeautifulSoup. First, I looked for the article's title and added it to a list I created named paragraphs. Then, I went through the article content searching for subtitles, quotes, and paragraphs, adding each piece of text to the list. In the end, I had a complete list with all the textual content of the article, which I then displayed. It's important to note that this method might not always be effective if the site changes its structure in the future. It's also essential to respect the site's terms of service when performing such tasks.
"""
...
# Importing necessary libraries and modules for NLP and deep learning tasks

# PyTorch for tensor operations
import torch

# Transformers library for state-of-the-art NLP models and tokenizers
...
# Natural Language Toolkit for various NLP tasks
import nltk
...
I chose the pretrained model named **"dbmdz/bert-large-cased-finetuned-conll03-english"**, which is a large-sized version of **BERT** fine-tuned specifically for the named **entity recognition task** in English using the CoNLL-03 dataset. I loaded this model using BertForTokenClassification.from_pretrained(). Additionally, I loaded the corresponding tokenizer for that model with BertTokenizer.from_pretrained(). With these tools, I set up a named entity recognition (NER) pipeline using the "simple" aggregation strategy.
"""
